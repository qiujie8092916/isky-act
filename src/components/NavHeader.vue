  ...
  >
    <div class="head-left rowcc">
      <template v-if="isBack || isClose">
        <Button :style="btnOpt" @onPress="_back" class="header-button rowcc">
          <IconFont
    ...
    <div class="head-right rowcc">
      <template v-if="shareParams">
        <Button :style="btnOpt" @onPress="_share" class="header-button rowcc">
...
export default class Header extends Vue {
  ...
  created(): void {
    ...
    if (this.$route.query.signalBarHeight) {
      this.signalBarHeight = parseFloat(
        this.$route.query.signalBarHeight as string
      );
    } else {
      Bridge.appGetSignalBarHeight(height => {
        this.signalBarHeight = height || 0;
      });
    }
  }
